...
typedef struct ms_vault_operation_t {
	sgx_status_t ms_retval;
	const uint8_t* ms_sealed_request_buffer;
	size_t ms_sealed_request_size;
	uint8_t* ms_sealed_response_buffer;
	size_t ms_sealed_response_capacity;
	size_t* ms_sealed_response_used;
...
static sgx_status_t SGX_CDECL sgx_enclave_create_report(void* pms)
{
	...
	uint8_t* _tmp_enclave_data = __in_ms.ms_enclave_data;
	...
	_in_retval = enclave_create_report((const sgx_target_info_t*)_in_p_qe3_target, _in_p_report, _in_enclave_data);
	...
	if (_in_enclave_data) {
		if (memcpy_verw_s(_tmp_enclave_data, _len_enclave_data, _in_enclave_data, _len_enclave_data)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
...
static sgx_status_t SGX_CDECL sgx_vault_operation(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_vault_operation_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_vault_operation_t* ms = SGX_CAST(ms_vault_operation_t*, pms);
	ms_vault_operation_t __in_ms;
	if (memcpy_s(&__in_ms, sizeof(ms_vault_operation_t), ms, sizeof(ms_vault_operation_t))) {
		return SGX_ERROR_UNEXPECTED;
	}
	sgx_status_t status = SGX_SUCCESS;
	const uint8_t* _tmp_sealed_request_buffer = __in_ms.ms_sealed_request_buffer;
	size_t _tmp_sealed_request_size = __in_ms.ms_sealed_request_size;
	size_t _len_sealed_request_buffer = _tmp_sealed_request_size * sizeof(uint8_t);
	uint8_t* _in_sealed_request_buffer = NULL;
	uint8_t* _tmp_sealed_response_buffer = __in_ms.ms_sealed_response_buffer;
	size_t _tmp_sealed_response_capacity = __in_ms.ms_sealed_response_capacity;
	size_t _len_sealed_response_buffer = _tmp_sealed_response_capacity * sizeof(uint8_t);
	uint8_t* _in_sealed_response_buffer = NULL;
	size_t* _tmp_sealed_response_used = __in_ms.ms_sealed_response_used;
	size_t _len_sealed_response_used = sizeof(size_t);
	size_t* _in_sealed_response_used = NULL;
	sgx_status_t _in_retval;

	if (sizeof(*_tmp_sealed_request_buffer) != 0 &&
		(size_t)_tmp_sealed_request_size > (SIZE_MAX / sizeof(*_tmp_sealed_request_buffer))) {
		return SGX_ERROR_INVALID_PARAMETER;
	}

	if (sizeof(*_tmp_sealed_response_buffer) != 0 &&
		(size_t)_tmp_sealed_response_capacity > (SIZE_MAX / sizeof(*_tmp_sealed_response_buffer))) {
		return SGX_ERROR_INVALID_PARAMETER;
	}

	CHECK_UNIQUE_POINTER(_tmp_sealed_request_buffer, _len_sealed_request_buffer);
	CHECK_UNIQUE_POINTER(_tmp_sealed_response_buffer, _len_sealed_response_buffer);
	CHECK_UNIQUE_POINTER(_tmp_sealed_response_used, _len_sealed_response_used);
	...
	if (_tmp_sealed_request_buffer != NULL && _len_sealed_request_buffer != 0) {
		if ( _len_sealed_request_buffer % sizeof(*_tmp_sealed_request_buffer) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		_in_sealed_request_buffer = (uint8_t*)malloc(_len_sealed_request_buffer);
		if (_in_sealed_request_buffer == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_sealed_request_buffer, _len_sealed_request_buffer, _tmp_sealed_request_buffer, _len_sealed_request_buffer)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}

	}
	if (_tmp_sealed_response_buffer != NULL && _len_sealed_response_buffer != 0) {
		if ( _len_sealed_response_buffer % sizeof(*_tmp_sealed_response_buffer) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		if ((_in_sealed_response_buffer = (uint8_t*)malloc(_len_sealed_response_buffer)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_sealed_response_buffer, 0, _len_sealed_response_buffer);
	}
	if (_tmp_sealed_response_used != NULL && _len_sealed_response_used != 0) {
		if ( _len_sealed_response_used % sizeof(*_tmp_sealed_response_used) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		if ((_in_sealed_response_used = (size_t*)malloc(_len_sealed_response_used)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_sealed_response_used, 0, _len_sealed_response_used);
	}
	_in_retval = vault_operation((const uint8_t*)_in_sealed_request_buffer, _tmp_sealed_request_size, _in_sealed_response_buffer, _tmp_sealed_response_capacity, _in_sealed_response_used);
	if (memcpy_verw_s(&ms->ms_retval, sizeof(ms->ms_retval), &_in_retval, sizeof(_in_retval))) {
		status = SGX_ERROR_UNEXPECTED;
		goto err;
	}
	if (_in_sealed_response_buffer) {
		if (memcpy_verw_s(_tmp_sealed_response_buffer, _len_sealed_response_buffer, _in_sealed_response_buffer, _len_sealed_response_buffer)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
	if (_in_sealed_response_used) {
		if (memcpy_verw_s(_tmp_sealed_response_used, _len_sealed_response_used, _in_sealed_response_used, _len_sealed_response_used)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	}
...
err:
	if (_in_sealed_request_buffer) free(_in_sealed_request_buffer);
	if (_in_sealed_response_buffer) free(_in_sealed_response_buffer);
	if (_in_sealed_response_used) free(_in_sealed_response_used);
	return status;
}
...
SGX_EXTERNC const struct {
	...
	4,
	{
		...
		{(void*)(uintptr_t)sgx_vault_operation, 0, 0},
